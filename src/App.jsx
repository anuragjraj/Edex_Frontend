...
function ProfilePage({ userId, currentUser, onMessage, onBack }) {
  const [data,setData]=useState(null); const [loading,setLoading]=useState(true); const [editMode,setEditMode]=useState(false); const [form,setForm]=useState({}); const [saving,setSaving]=useState(false); const [ok,setOk]=useState(false)
  const isOwn = !userId || userId===currentUser?.id
  const targetId = userId||currentUser?.id
  useEffect(()=>{
    setLoading(true)
    api.get(`/api/profiles/${targetId}`).then(d=>{setData(d);setForm(d.profile||{});setLoading(false)}).catch(()=>setLoading(false))
  },[targetId])
  const save=async()=>{
    setSaving(true); setOk(false)
    try{ await api.put('/api/profiles/me',form); setData(d=>({...d,profile:form})); setOk(true); setTimeout(()=>{setEditMode(false);setOk(false)},1500) }
  ...
  return (
    <div style={{ padding:24, width:'100%', boxSizing:'border-box', fontFamily:"'Nunito',sans-serif", maxWidth:780, margin:'0 auto' }}>
      ...
      <div style={{ position:'relative', marginBottom:56 }}>
        <div style={{ height:130, borderRadius:'14px 14px 0 0', background:prof.banner_url?`url(${prof.banner_url}) center/cover`:'linear-gradient(135deg,#4338ca,#6366F1,#8B5CF6)', overflow:'hidden' }}/>
        <div style={{ position:'absolute', bottom:-44, left:24, width:80, height:80, borderRadius:'50%', background:'linear-gradient(135deg,#6366F1,#8B5CF6)', border:'3px solid var(--bg)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:32, fontWeight:900, color:'#fff', fontFamily:"'Sora',sans-serif" }}>
          {user?.name?.[0]?.toUpperCase()||'?'}
        </div>
        {!isOwn&&onMessage&&<button onClick={()=>onMessage(user.id)} style={{ position:'absolute', bottom:-36, right:24, padding:'8px 20px', borderRadius:10, border:'none', background:'var(--accent)', color:'#fff', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:"'Nunito',sans-serif" }}>💬 Message</button>}
        {isOwn&&<button onClick={()=>editMode?save():setEditMode(true)} style={{ position:'absolute', bottom:-36, right:24, padding:'8px 18px', borderRadius:10, border:'1px solid var(--border)', background:'var(--bg2)', color:'var(--text-h)', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:"'Nunito',sans-serif" }}>
          {saving?'Saving...':editMode?'✓ Save':'✏️ Edit'}
        </button>}
      </div>
      {ok&&<SuccessMsg msg="Profile saved!"/>}
      {/* Info card */}
      <Card style={{ marginBottom:14 }}>
        <h2 style={{ fontFamily:"'Sora',sans-serif", fontWeight:900, color:'var(--text-h)', margin:'0 0 4px', fontSize:22 }}>{user?.name}</h2>
        {editMode
          ?<input value={form.headline||''} onChange={e=>setForm(f=>({...f,headline:e.target.value}))} placeholder="Add a headline..." style={{ ...T.input, marginBottom:8 }}/>
          :<p style={{ color:'var(--text)', fontSize:14, margin:'0 0 12px' }}>{prof.headline||`${user?.role} at ${user?.schools?.name||'School'}`}</p>
        }
        <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
          <span style={{ background:'var(--accent-bg)', color:'var(--accent)', border:'1px solid var(--accent-border)', borderRadius:20, padding:'3px 12px', fontSize:12, fontWeight:700 }}>🎓 {user?.class_level||user?.role}</span>
          {rank?.rank&&<span style={{ background:'rgba(245,158,11,.1)', color:'#FCD34D', border:'1px solid rgba(245,158,11,.2)', borderRadius:20, padding:'3px 12px', fontSize:12, fontWeight:700 }}>🏆 #{rank.rank} of {rank.total_users?.toLocaleString()}</span>}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:14 }}>
        {[['🔥',stats.current_streak||0,'Streak'],['🤔',stats.doubts_solved||0,'Doubts'],['🎯',stats.quizzes_done||0,'Quizzes'],['📖',stats.notes_made||0,'Notes']].map(([e,v,l])=>(
      ...
      <Card style={{ marginBottom:14 }}>
        ...
        {editMode
          ?<textarea value={form.about||''} onChange={e=>setForm(f=>({...f,about:e.target.value}))} placeholder="Tell your story..." rows={4} style={{ ...T.input, resize:'vertical' }}/>
          :<p style={{ color:'var(--text)', fontSize:14, lineHeight:1.7, margin:0 }}>{prof.about||'No about section yet.'}</p>
        }
      </Card>
      {/* Skills */}
      <Card>
        <h4 style={{ fontFamily:"'Sora',sans-serif", fontWeight:800, color:'var(--text-h)', margin:'0 0 12px', fontSize:15 }}>Skills</h4>
        {editMode&&<input value={(form.skills||[]).join(', ')} onChange={e=>setForm(f=>({...f,skills:e.target.value.split(',').map(s=>s.trim()).filter(Boolean)}))} placeholder="Mathematics, Physics, Python..." style={{ ...T.input, marginBottom:10 }}/>}
        <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
          {(prof.skills||[]).map((s,i)=><span key={i} style={{ background:'var(--accent-bg)', color:'var(--accent)', border:'1px solid var(--accent-border)', borderRadius:20, padding:'4px 12px', fontSize:12.5, fontWeight:700 }}>{s}</span>)}
          {(prof.skills||[]).length===0&&<span style={{ color:'var(--text)', fontSize:13 }}>No skills added yet.</span>}
        </div>
      </Card>
    </div>
  )
